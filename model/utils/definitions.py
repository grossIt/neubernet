...
class YieldNet(NeuberNetEnsemble):
    def __init__(
        ...
        self.register_buffer("yield_log_eps", yield_log_eps)
    ...
    def forward(self, x):
        if self.normalize:
            ...
            x[..., 0] = torch.exp(x[..., 0] - self.yield_log_eps)
            return x
        else:
            return torch.cat([component(x) for component in self.components], dim=1)
            ...
            if self.yieldnet is not None and self.signsvm is not None:
                ...
                x = torch.cat(
                    [
                        ...
                        x[:, 2 * (self.branch_input_dim // 3) :] * sign_x[:, 1],
                    ],
                    dim=1,
                )

                yield_x = self.yieldnet(x[:, :-2])
                scaling_factors = torch.zeros(yield_x.shape[0])
                # When 0 < yield_x[:, 0] <= 1, scaling_factors are 1/yield_x[:, 0]
                # When 1 < yield_x[:, 0] <= yield_x[:, 1], scaling_factors are 1
                # When yield_x[:, 1] < yield_x[:, 0], scaling_factors are NaNs
                scaling_factors[(yield_x[:, 0] > 0) & (yield_x[:, 0] <= 1)] = (
                    1 / yield_x[(yield_x[:, 0] > 0) & (yield_x[:, 0] <= 1), 0]
                )
                scaling_factors[
                    (yield_x[:, 0] > 1) & (yield_x[:, 0] <= yield_x[:, 1])
                ] = 1
                scaling_factors[yield_x[:, 1] < yield_x[:, 0]] = float("nan")
                ...
                # Where scaling factors are > 1, the analysis is elastic, so indexes 1 and 8:13 are set to 0
                x[scaling_factors > 1, 1] = 0
                x[scaling_factors > 1, 8:13] = 0
                ...
                # If any of the scaling factors are NaN, the analysis is invalid (small-scale plasticity is violated)
                # In this case, we issue a warning (the output is already NaNs)
                if torch.isnan(scaling_factors).any():
                    print(
                        "Warning: small-scale plasticity is violated in some analyses. In these cases, the output is NaNs"
                    )
